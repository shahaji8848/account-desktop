...
import { paymentTypeList, referenceCategory, referenceTypes } from '../../utils/receiptFormData';
...
import { getAllAccounts, paymentEntryAccountsDetails } from '../../../apis/payment_entry_apis';
import { getData, postData } from '../../../apis/util';
...
const ReceiptTable = ({ homeHookData, globalData, companyGstin }: any) => {
  ...
  const [refernceID, setReferenceID] = useState<any>('');
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>, id: any, rowType?: any) => {
    ...
    if (rowType === 'entry_row') {
      ...
      setReferences((prevReferences: any) =>
        prevReferences.map((reference: any) => (reference.id === id ? { ...reference, [name]: updatedValue } : reference))
      );
    }
  };

  // Handle key press (Enter to add new row, Escape to close modal)
  const handleKeyDown = async (e: any, field: any, id: any, referenceId?: any) => {
    ...
    if (e.key === 'Escape') {
      ...
      if (field === 'party_type' || field === 'party' || field === 'payment_type' || field === 'cost_center') {
        setEntries((prevEntries: any) =>
          prevEntries.map((entry: any) => (entry.id === id ? { ...entry, [field]: currentFilterList[selectedIndex]?.name || '' } : entry))
        );
        ...
        try {
          const paidToAccountResponse = window.electron
            ? await window.electron.paymentEntryAccountsDetails({
                doctype: 'dummy',
        ...
        setReferences((prevEntries: any) =>
          prevEntries.map((entry: any) => (entry.id === referenceId ? { ...entry, [field]: currentFilterList[selectedIndex]?.name || '' } : entry))
        );

        if (field === 'reference_doctype') {
          const selectedRowData = entries.find((entry: any) => entry.id === id);
          const filters = {
            company: companyName,
            ref_type: currentFilterList[selectedIndex]?.name || '',
            party_type: selectedRowData?.party_type,
            party: selectedRowData?.party,
            party_account: selectedRowData?.party_account,
  ...
  const handleInputFocus = async (e: React.FocusEvent<HTMLInputElement>, id: any) => {
    ...
    if (field === 'party_type') {
      ...
      try {
        ...
        const fetchedPartyList = window.electron
      ...
      setSelectedIndex(0);
    } else if (field === 'agst_ref') {
      setShowFilter(true);
      setCurrentFilterList(referenceCategory);
      setSelectedIndex(0);
    } else if (field === 'reference_doctype') {
      setShowFilter(true);
      setCurrentFilterList(referenceTypes);
      setSelectedIndex(0);
  ...
  const updateReferenceById = (id: any, newData: any) => {
    setReferences((prevReferences: any) =>
      prevReferences.map((ref: any) =>
        ref.id === id
          ? {
              ...ref,
              reference_name: newData.name,
              total_amount: newData.grandTotal,
              outstanding_amount: newData.outstanding,
              allocated_amount: newData.allocated,
            }
          : ref
      )
    );
  ...
  const updateEntryWithReferences = (entryId: string, newReferences: any[]) => {
    setEntries((prevEntries: any) => {
      // Collect all already stored reference IDs across all entries
      const allStoredReferenceIds = new Set(prevEntries.flatMap((entry: any) => entry.stored_references_id));

      const updatedEntries = prevEntries.map((entry: any) => {
        if (entry.id === entryId) {
          // Filter only references that are NOT in any previous entries
          const uniqueReferences = newReferences.filter((ref) => !allStoredReferenceIds.has(ref.id));

          return {
            ...entry,
            stored_references_id: [...entry.stored_references_id, ...uniqueReferences.map((ref) => ref.id)],
            references: [...entry.references, ...uniqueReferences],
          };
        }
        return entry;
      });
  ...
  // console.log("entries", entries)
  return (
    <div className={`container-fluid ${styles.journal_table_container}`} ref={formRef}>
        ...
        <div className="col-4">
          <div className="row">
            <div className="col-12 d-flex align-items-center">
              <label className="fw-bold" style={{ flexBasis: '20%' }}>
                Account
              </label>
      ...
      {/* Header Row */}
      <div className={`row py-2 fw-bold`} style={{ border: '1px solid lightgray' }}>
                ...
                <div className="col-12 d-flex">
                  <div className={`text-muted text-center`}>
                    <input
                      ...
                      // style={{ width: '20%' }}
                      name="agst_ref"
                      value={reference?.agst_ref}
                      placeholder="Agst Ref"
                      onChange={(e) => handleInputChange(e, reference.id, 'reference_row')}
                      onKeyDown={(e) => handleKeyDown(e, 'agst_ref', entry.id, reference.id)}
                  ...
                  <div className={`text-muted text-center`}>
                    <input
                      ...
                      onChange={(e) => handleInputChange(e, reference.id, 'reference_row')}
                      onKeyDown={(e) => handleKeyDown(e, 'reference_doctype', entry.id, reference.id)}
                      onFocus={(e) => handleInputFocus(e, reference.id)}
                    />
                  </div>
                  ...
                  <div className={`text-muted text-center`}>
                    <input
                      ...
                      placeholder="Name"
                      onChange={(e) => handleInputChange(e, reference.id, 'reference_row')}
                      onKeyDown={(e) => handleKeyDown(e, 'reference_name', entry.id, reference.id)}
                      onFocus={(e) => handleInputFocus(e, reference.id)}
                    />
                  </div>
                  ...
                  <div className={`text-muted text-center`}>
                    <input
                      ...
                      onChange={(e) => handleInputChange(e, reference.id, 'reference_row')}
                      onKeyDown={(e) => handleKeyDown(e, 'total_amount', entry.id, reference.id)}
                      onFocus={(e) => handleInputFocus(e, reference.id)}
                    />
                  </div>
                  ...
                  <div className={`text-muted text-center`}>
                    <input
                      ...
                      onChange={(e) => handleInputChange(e, reference.id, 'reference_row')}
                      onKeyDown={(e) => handleKeyDown(e, 'outstanding_amount', entry.id, reference.id)}
                      onFocus={(e) => handleInputFocus(e, reference.id)}
                    />
                  </div>
                  ...
                  <div className={`text-muted text-center`}>
                    <input
                      ...
                      onChange={(e) => handleInputChange(e, reference.id, 'reference_row')}
                      onKeyDown={(e) => handleKeyDown(e, 'allocated_amount', entry.id, reference.id)}
                      onFocus={(e) => handleInputFocus(e, reference.id)}
                    />
                  </div>
                </div>
      ...
      {/* Filter Dropdown */}
      {showFilter && (
        <ShowFilter filteredItems={currentFilterList} selectedIndex={selectedIndex} handleItemFocus={(index: number) => setSelectedIndex(index)} />
      )}
      ...
      <ReferenceTablePopUp
        ...
        handleKeyDown={handleKeyDown}
        handleInputFocus={handleInputFocus}
        updateReferenceById={updateReferenceById}
      />

      {/* Quit Confirmation Modal */}
      {isQuitModalOpen && (
        <QuitConfirmationModal type="receipt_form" isOpen={isQuitModalOpen} setIsQuitModalOpen={setIsQuitModalOpen} homeHookData={homeHookData} />
      )}
    </div>
  );
};
