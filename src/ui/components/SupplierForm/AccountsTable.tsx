...
import '../CustomerForm/AccountsTable.css';
...
const AccountsTable = ({
  ...
  const fetchAccountList = async (company: any) => {
    const x = window.electron
      ? await window.electron.getData({
          doctype: 'Account',
          filters: { root_type: 'Asset', account_type: 'Receivable', company: company, is_group: 0 },
          token,
        })
      : await getData({
          doctype: 'Account',
  ...
  const handleInputChange = (id: any, field: any, value: any) => {
    ...
    const newRows = updatedRows.map((row) => (row.id === id ? { ...row, [field]: value } : row));
  ...
  const handleKeyDown = (e: any, field?: any, id?: any) => {
    ...
    if (e.ctrlKey && e.key === 'Enter') {
      ...
      const newIndex = e.key === 'ArrowDown' ? (selectedIndex + 1) % list.length : (selectedIndex - 1 + list.length) % list.length;
  ...
  return (
    <div className="p-4">
      ...
      {selectedRows.length > 0 && (
        <button className="btn btn-danger" onClick={deleteSelectedRows} onKeyDown={(e: any) => handleKeyDown(e, 'delete_row')}>
          Delete
        </button>
      )}
    </div>
