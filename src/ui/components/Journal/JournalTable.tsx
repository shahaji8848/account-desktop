...
const JournalTable = ({ homeHookData, globalData, companyGstin }: any) => {
      ...
      if (field === 'particulars') {
        setEntries((prevEntries: any) => {
          return prevEntries.map((entry: any) => {
            return entry.id === id ? { ...entry, [field]: currentFilterList[selectedIndex]?.name || '' } : entry;
          });
        });
        //fetching current balance for selected account
        try {
          const accountBalance = window.electron
            ? await window.electron.getData({
                doctype: 'Journal Entry Accounts',
                filters: { account: currentFilterList[selectedIndex]?.name, company: companyName },
                token,
              })
            : await getData({
                doctype: 'Journal Entry Accounts',
        ...
        if (field === 'reference_name') {
          setEntries((prevEntries: any) =>
            prevEntries.map((entry: any) => (entry.id === id ? { ...entry, [field]: referenceNameLsit[selectedIndex]?.name || '' } : entry))
          );
        } else if (field === 'type') {
          setEntries((prevEntries: any) =>
            prevEntries.map((entry: any) =>
  ...
  return (
    <div className={`container-fluid ${styles.journal_table_container}`} ref={formRef}>
        ...
        <div key={index} className={`row align-items-center ${styles.entryRow}`}>
          ...
          <div className={`col-1 d-flex justify-content-center fw-bold ${styles.voucherRowActive}`} style={{ height: '20px' }}>
            {entry.debit === 'disabled' ? (
              <span style={{ visibility: 'hidden' }}>debit</span>
            ) : (
              <input
                type="text"
          ...
          <div className={`col-1 d-flex justify-content-center ${styles.voucherRowActive}`} style={{ height: '20px' }}>
            {entry.credit === 'disabled' ? (
              <span style={{ visibility: 'hidden' }}>credit</span>
            ) : (
              <input
                type="text"
                className={`form-control text-center p-0 ${styles.voucherRowActive}`}
          ...
          <div className="col-3">
            <div className={`text-muted text-center `}>
              <p>
                {' '}
                Cur Bal:{' '}
                <span className={`fst-italic ${entry.curBalance < 0 ? styles.negativeBalance : styles.curBal}`}>
                  {Math.abs(entry.curBalance)?.toFixed(2)}
                </span>
              </p>
            </div>
          </div>
      ...
      {isQuitModalOpen && (
        <QuitConfirmationModal type="journal_form" isOpen={isQuitModalOpen} setIsQuitModalOpen={setIsQuitModalOpen} homeHookData={homeHookData} />
      )}
    </div>
  );
};
