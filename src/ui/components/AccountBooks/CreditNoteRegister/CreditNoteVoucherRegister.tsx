...
import { creditNoteBreakupReport } from '../../../../apis/reports/credit_note_register';

const CreditNoteVoucherRegister = ({ homeHookData, globalData }: any) => {
  ...
  const fetchVoucherList = async () => {
    try {
      let x = window.electron
        ? await window.electron.creditNoteBreakupReport({
            filters: {
    ...
    <div className="w-100 d-flex align-items-stretch justify-content-between" style={{ overflow: 'hidden' }}>
      <div className="main-body" style={{ width: '86%' }}>
        ...
        <VoucherRegister
          ...
          type="credit_note_voucher_register"
        />
        <BottomNavbar />
      </div>
...
export default CreditNoteVoucherRegister;
