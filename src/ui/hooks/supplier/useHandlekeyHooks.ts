...
const useHandlekeyHooks = ({
    ...
    const handleKeyDown = async (e: any, field?: any, type?: any, payload?: any) => {
        ...
        if (e.ctrlKey && e.key === "Enter") {
            ...
            e.preventDefault();
            if (type === 'supplier') {
                setSupplierFormData({ ...supplierFormData, [currentField]: currentFilterList[selectedIndex]?.name ? currentFilterList[selectedIndex]?.name : currentFilterList[selectedIndex] })
            ...
            // homeHookData?.setShowSupplierForm(false)
        }
    ...
    const handleInputFocus = (e: React.FocusEvent<HTMLInputElement>) => {
        ...
        setShowFilter(false);
        if (field === 'supplier_type' || field === 'tax_withholding_category' || field === 'gst_category' || field === 'gstCategory' || field === 'addressType' || field === 'bank' || field === 'account_type' || field === 'default_currency') {
            setShowFilter(true);
            setCurrentField(field);
            setSelectedIndex(0)
        }
